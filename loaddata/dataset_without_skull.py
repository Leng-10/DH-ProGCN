...
class Train_Dataset(data.Dataset):
    def __init__(self, root, LABELS, transforms):
        ...
        self.LABELS = LABELS
        labels1 = {name: index for index in range(len(self.LABELS)) for name in
                   glob.glob(self.root + '/' + self.LABELS[index] + '/train' + '/*.nii.gz')}
...
class Test_Dataset(data.Dataset):
    def __init__(self, image_dir, LABELS, transform):
        ...
        self.LABELS = LABELS
        labels1 = {name: index for index in range(len(self.LABELS)) for name in
                   glob.glob(self.root + '/' + self.LABELS[index] + '/test' + '/*.nii.gz')}
        # labels1 = {name: index for index in range(len(LABELS)) for name in
